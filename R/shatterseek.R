...
setMethod("initialize", "SVs", function(.Object, ...) {
	 ...
	 #if(length(.Object@sample)==0) .Object@sample = rep("",length(.Object@chrom1))
	...
	#.Object@chromothEvent = rep(0,length(.Object@chrom1))
        .Object
       })
...
###---------------------------------------------------------------------------
### define a vector of probabilities for each chromosome in the reference set
### sort the reference set
##SV.tcga.pancer$chr_num <- SV.tcga.pancer[,1]
##SV.tcga.pancer$chr_num[SV.tcga.pancer$chr_num=="X"] <- "23"
##SV.tcga.pancer$chr_num <- as.numeric(SV.tcga.pancer$chr_num)
##SV.tcga.pancer <- SV.tcga.pancer[order(SV.tcga.pancer$chr_num,decreasing=F),]
##SV.tcga.pancer$chr_num <- NULL
##
##ratios <- table(chrom1) / length(chrom1)
##
...
shatterseek = function(SV.sample,seg.sample,min.Size=1){#,cores=20,B=5000,filter=TRUE,poisson=TRUE,mean.poisson,method="Ruibin",cancer.type=""){
